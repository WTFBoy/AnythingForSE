...
<h3>非阻塞式I/O<h3>
非阻塞式套接字是在通知内核：当所请求的I/O操作非得把本进程投入睡眠才能完成时， 不要把本进程投入睡眠,而是返回一个错误。举个例子，如下图所示，![img](http://https://github.com/WTFBoy/AnythingForSE/blob/main/1.png).<br>
...
<h3>I/O复用模型</h3>
借助I/O复用，可以借助select或Poll，阻塞在这两个调用上，而不是阻塞在真正的I/O系统调用上。即有目的的调用。![img](https://github.com/WTFBoy/AnythingForSE/blob/main/PrimeC6-3.png)
<h3>信号驱动式I/O</h3>
